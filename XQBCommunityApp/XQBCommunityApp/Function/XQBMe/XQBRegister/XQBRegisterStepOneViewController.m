//
//  XQBRegisterStepOneViewController.m
//  XQBCommunityApp
//
...
@interface XQBRegisterStepOneViewController () <CustomTextFieldDelegate>
...
@implementation XQBRegisterStepOneViewController
...
- (void)requestUsersRegistrationVcode
{
    ...
    [parameters setObject:VERIFICATION_CODE_TYPE_REGISTER forKey:@"serviceType"];
...
- (void)requestUsersRegistrationPhone
{
    ...
    [manager POST:API_USER_VERIFY_REGISTER_VCODE_URL parameters:parameters success:^(AFHTTPRequestOperation *operation, id responseObject) {
        XQBLog(@"\nresponseObject:%@", responseObject);
        if ([[responseObject objectForKey:XQB_NETWORK_ERROR_CODE] isEqualToString:XQB_NETWORK_ERROR_CODE_OK]) {
            XQBLog(@"验证成功");            
            XQBRegisterStepTwoViewController *registerStepTwoVc = [[XQBRegisterStepTwoViewController alloc] init];
            registerStepTwoVc.phomeNumberString = [_phoneTextField getMiddleTextFieldText];
            [self.navigationController pushViewController:registerStepTwoVc animated:YES];
        } else if ([[responseObject objectForKey:XQB_NETWORK_ERROR_CODE] isEqualToString:XQB_NETWORK_VRV_VERIFICATION_CODE_INCORRECT]) {
            XQBLog(@"验证码不正确");
            [self.view.window makeCustomToast:[responseObject objectForKey:XQB_NETWORK_ERROR_MESSAGE]];
        } else if ([[responseObject objectForKey:XQB_NETWORK_ERROR_CODE] isEqualToString:XQB_NETWORK_VRV_VERIFICATION_CODE_NOT_EXISTT]) {
            XQBLog(@"验证码不存在");
            [self.view.window makeCustomToast:[responseObject objectForKey:XQB_NETWORK_ERROR_MESSAGE]];
        } else {
            //加载服务器异常界面
