...
-(void)drawRect:(CGRect)rect {
  ...
  if((self.badgeValue != 0 || self.displayWhenZero) && self.badgeType == kBadgeValue) {
    CGContextRef context = UIGraphicsGetCurrentContext();
  ...
  } else if (self.badgeType == kBadgeCircle && self.displayWhenZero){
      CGContextRef context = UIGraphicsGetCurrentContext();
      [self.badgeColor set];
      CGContextFillEllipseInRect(context, CGRectInset(rect, 1, 1));
  }
